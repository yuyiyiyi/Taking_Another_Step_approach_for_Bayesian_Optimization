...
lengthscale_n=size(theta0,2);
if lengthscale_n>1
    options = optimoptions('fmincon','Algorithm','sqp','MaxFunctionEvaluations',20*lengthscale_n,'OptimalityTolerance',1E-20,'StepTolerance',1E-20,'Display','off');
else
...
try
    ...
    % if can't cholosky, use approximate approach
    R=nearestSPD(R);
    % use the Cholesky factorization
    L = chol(R,'lower');
end
